    ...
    def run_case(self):
        ...
        for test_time in range(self.TEST_TIME):
            ...
            try:
                ...
                time.sleep(2)
                self._log_step('3、点击蜜雪冰城小程序')
                self._click('蜜雪冰城', 0.65, 0.20, contains=True, wait=5)
                self._log_step('4、点击点餐')
                self._click('点餐', 0.366, 0.927, contains=True)
                self._log_step('5、浏览点餐页面，上滑3次、下滑3次')
                self._browse(3)
